...
{
    return serial_write(((modbus_rtu_t*)ctx->backend_data)->serial, req, req_length, ctx->response_timeout.tv_sec);
}
...
{
    return serial_read(((modbus_rtu_t*)ctx->backend_data)->serial, rsp, rsp_length, ctx->response_timeout.tv_sec);
}
...
{
    return serial_open(((modbus_rtu_t*)ctx->backend_data)->serial);
}
...
{
    serial_close(((modbus_rtu_t*)ctx->backend_data)->serial);
}

static int _modbus_rtu_flush(modbus_t *ctx)
{
    return serial_clean(((modbus_rtu_t*)ctx->backend_data)->serial);
}
...
static void _modbus_rtu_free(modbus_t *ctx) {
    serial_destory(((modbus_rtu_t*)ctx->backend_data)->serial);
...
modbus_t* modbus_new_rtu(const char *device, uint8_t index,
                         ...
                         int stop_bit)
{
    ...
    ctx_rtu->serial = serial_new(index, baud, parity, (uint8_t)data_bit, (uint8_t)stop_bit, 1);

    assert(ctx_rtu->serial);
